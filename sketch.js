...
function setup() {
    ...
    wallR = createSprite(width-450, height/2, 20,height)
    ...
    gameOver = createSprite(width/2, height-(height-100), 100,20)
...
function draw() {
    ...
    if(gameState===start){
        textSize(30);
        text("Use arrow keys to play \n Press space to start", width/2-160,100)
    ...
    if(gameState===end){
        ...
        text("Press space to restart", width/2-150, height-(height-160));
        if(keyWentDown("space")){
            userCar.x = width/2;
            userCar.y = height/2;
            distance = 0;
